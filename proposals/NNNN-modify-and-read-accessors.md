...
The `UniqueRead.property { read }` accessor is a "single-yield coroutine".
...
Like `UniqueRead.property { read }` above, `GetModify.property { modify }` is a single-yield coroutine.
...
In general, the rules for yields in single-yield coroutines are similar to those of deferred initialization of `let` variables:
it must be possible for the compiler to guarantee there is exactly one yield on every path.
In other words, there must not be a path through the single-yield coroutine's body with either zero[^1] or more than one yield.
...
In the future, it may be desirable to enable functions to yield:
...
The intervening years of experience with the feature has not borne out.
