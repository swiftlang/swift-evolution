...
operations where necessary to resolve ambiguity with a mathematical structural operation:
`.==`, `.!=`, `.*`, and `.&*`.

On the whole, I think using unadorned operators is clearer in use, but the `.` prefixes are also
tolerable, and there is precedent for this choice.
...
4. I have updated the Mask types to use `&&` and `||` instead of `&` and `|`. Neither option is
quite ideal; `&&` is misleading because it does not short-circuit like the `Bool` operator does,
but on the other hand `&` has the wrong operator precedence. We may want to use something
like `.&` and `.|` here instead.
