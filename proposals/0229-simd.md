...
* Status: **Scheduled for review (September 27...October 5, 2018)**
...
confusing, I've updated the implementation branch to use `.`-prefixed operators for all
lanewise operations. This also has the pragmatic benefit of resolving come concerns about
the complexity of typechecking SIMD operations.

On the whole, I think using unadorned operators is clearer in use, but the `.` prefixes are also
tolerable, and there is precedent for this choice: Julia does exactly the same thing. The main
downside is that this is a *lot* of new operators to be adding.
...
There is also a set of protocols defining the vector sizes: `Vector[2,3,4,8,16,32,64]`.
...
2. As mentioned above, `.`-prefixes have been added to *all* elementwise operations. I see
pretty good arguments both in favor of and against this change. I think that it largely comes
down to a matter of taste, but there are some genuine pragmatic arguments in favor of using
the `.`. I'm quite torn on this, however.
