...
* Both `Type<T>` and `AnyType<T>` will be declared in the Swift standard library even if part of the implementation might be compiler magic. That move is really important to allow developers to create custom *nested* types named as `Type`. `ModuleName.TypeName.Type` is ambiguous today, but will become possible after this proposal. That means `Type<T>` and `AnyType<T>` will be shortcuts for `Swift.Type<T>` and `Swift.AnyType<T>`.
