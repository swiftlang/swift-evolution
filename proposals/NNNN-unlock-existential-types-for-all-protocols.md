...
Existential types are types generated by the compiler that can wrap any value of a type which conforms to a given _protocol_, _class_, or _composition of the above_. They are an incredibly useful feature in Swift for specific use cases that involve dynamicity. Being able to wrap different types that conform to a specified protocol, users and, more often, library authors can alleviate a lot of boilerplate code. That is, in certain use cases creating a custom existential type can be quite a daunting task due to its complexity; not to mention, that it introduces a lot of difficult-to-understand and hard-to-maintain code.
...
Libraries often need to provide custom existential types for commonly-used protocols, when their generation is unavailable. However, that  introduces hard-to-understand and difficult-to-maintain boilerplate code into the codebase, not to mention that it imposes resilience limitations from the increase API and ABI surfaces, as well. As a result, library authors have to spend valuable time on creating, documenting and maintaining such code, which can easily be generated by the compiler. For instance, [SwiftUI](https://developer.apple.com/documentation/swiftui) relies heavily on the ubiquitous [`View`](https://developer.apple.com/documentation/swiftui/view) protocol. This `View` protocol has a public interface that approximately looks like this:
