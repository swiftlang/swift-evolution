...
In Swift, an existential type for a given protocol or protocol composition is a distinct type that can wrap a value of any conforming type. They are an incredibly useful feature in Swift for specific use cases that involve dynamicity. Being able to wrap different types that conform to a specified protocol, users and, more often, library authors can alleviate a lot of boilerplate code. That is, in certain use cases creating a custom existential type can be quite a daunting task due to its complexity; not to mention, that it introduces a lot of difficult-to-understand and hard-to-maintain code.
