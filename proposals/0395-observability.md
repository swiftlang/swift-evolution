...
*FIXME*: Deleted a bit too much here! Examples??
...
- because `Observable` is a marker protocol, subclasses just work™
- ***Note:*** can an `Observable` type inherit from a non-`Observable` type?
...
In the example above, the `render` function accesses each car's `name` property. When any of the cars change `name`, the `onChange` closure is then called on the first change. However, if a car has an award added, the `onChange` call won't happen. This design supports uses that require implicit observation tracking, ensuring that views are only updated in response to relevant changes.
...
***ADD:*** Near-future direction that would allow properties without default values
