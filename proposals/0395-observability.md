...
For read-only computed properties, the static `dependencies(of:)` method is used to indicate additional key paths from which the property is computed. This is similar to the mechanism that KVO uses to provide additional key paths that have effects to key paths. (See [Macro Synthesis](#macro-synthesis) for more information)
...
Since all the code generated by the macro could be manually written, developers can write their own implementation when they need more fine-grained control. For example, the following `Model` type is annotated with the `@Observable` macro:
...
Properties that have `willSet` and `didSet` property observations are supported. For example, the `@Observable` macro on the `PropertyObserverExample` type here:

```swift
@Observable final class PropertyObserverExample {
    var a: Int {
...
Observation of computed properties is supported, but stored property dependencies cannot be determined during synthesis due to macro limitations (see [Future Directions](#future-directions)). For example, the `@Observable` macro on the `ComputedPropertyExample` type here:

```swift
@Observable final class ComputedPropertyExample {
    var a: String { "\(b)" }
    var b: Int = 0
    var c: String = ""
}
...
... generates an implementation for the `dependencies(of:)` requirement that creates a list of all the type's stored properties, and then returns that as the set of dependencies for any computed property. For the `ComputedPropertyExample` type above, that implementation looks like this:
...
Future macro features could potentially permit refinements to the `dependencies(of:)` implementation generated by the `@Observable` macro, to more accurately select the stored properties that each computed property depends on instead of declaring dependency on all stored properties.
