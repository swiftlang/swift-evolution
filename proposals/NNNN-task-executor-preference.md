# Design: Task Executors / Executor Preference
...
As Swift concurrency is getting adopted in more performance sensitive codebases, it has become clear that the lack of control over where nonisolated functions execute is a problem. Users are not able to influence or prevent this default, defensive, behavior of hopping to the global poo even when such hops are undesirable.
...
Task(on: executor) { 
  try await capy.eat() // executes on Capybara actor's executor, as expected
}
```
...
The task executor preference can be inspected dynamically using the `Task.executorPreference` accessor. This should not usually be necessary because the runtime will perform all the expected executor hopping for us, however it may sometimes be useful to inspect or confirm out expectations on task executor semantics:

```
extension Task { 
  var executorPreference: (any Executor)? { get }
}
...
This can be used to manually propagate an executor to different APIs, such as when the necessity to use an unstructured task arrises, but we’d like to keep the executor preference:

```
Task(on: Task.executorPreference) {
  // keeps the preference, 
  // even though unstructured tasks don't inherit it automatically
}
```
...
Task(on: specialExecutor) {
  _ = await withTaskGroup(of: Int.self) { group in 
    group.addTask() { 
      // guaranteed to run on the inherited specialExecutor
      12 
    }
    group.addTask(on: specialExecutor) { 
      // guaranteed to run on `specialExecutor`
      42 
    }
...
I’d like to thanks Franz Bunsch for detailed analysis of this approach in Swift-NIO based systems as well as its relation to AsyncSequences. 
