...
For each target that defines at least one resource file, SwiftPM will produce
...
SwiftPM will generate an internal static extension on `Bundle` for each module it
compiles:
...
Because this is an internal static property , it would be visible only to code
within the same module, and the implementations for each module would not
interfere with each other. The implementation generated by SwiftPM would use
information about the layout of the built product to instantiate and cache the
bundle that contains the resources.

The first access to the `moduleResources` property would cause the bundle to be
...
let path = Bundle.moduleResources.path(forResource: "DefaultSettings", ofType: "plist")

// Load an image that can be in an asset archive in a bundle.
let image = UIImage(named: "MyIcon", in: Bundle.moduleResources, compatibleWith: UITraitCollection(userInterfaceStyle: .dark))

// Find a vertex function in a compiled Metal shader library.
let shader = try mtlDevice.makeDefaultLibrary(bundle: Bundle.moduleResources).makeFunction(name: "vertexShader")

// Load a texture.
let texture = MTKTextureLoader(device: mtlDevice).newTexture(name: "Grass", scaleFactor: 1.0, bundle: Bundle.moduleResources, options: options)
```

Because the name of the accessor would always be `moduleResources`, code could
...
`PACKAGE_MODULE_BUNDLE` which can be used to access the bundle from any `.m` file.
...
This design aims to reduce that risk as much as possible by limiting the
...
In the long run, we would like to do better.  Because SwiftPM knows the names
