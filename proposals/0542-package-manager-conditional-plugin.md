...
SwiftPM can evaluate the host and trait filters before it invokes the build tool plugin. A target-platform filter works differently. The build tool plugin and the Project Interchange Format (PIF) do not use the build request. Thus, the build tool plugin does not know the target platform. SwiftPM applies the target-platform filter to build commands and to the generated outputs from the plugin usage. A target-platform filter does not guarantee that a prebuild command will not execute. PIF-based builds run prebuild commands while generating the request-independent PIF so that SwiftPM can discover their outputs. SwiftPM applies the filter to those discovered outputs. The build system then uses the filter for each configured target.
...
SwiftPM applies the target-platform condition to all build commands and generated files from the plugin usage. For prebuild commands, the condition controls whether the discovered outputs are added to the configured target; it does not guarantee that the prebuild command itself will not execute. On an excluded platform, SwiftPM does not add the generated files to the target. The target must compile without these files. To use a different generator, add a separate plugin usage with its own condition:
