...
The author then concludes that although it's possible for developers to feel confused by the syntax, this is something that already exists with `#available` and has never/cannot result in the feature being misused. As `#unavailable` does not introduces any new functionality that could change this, this issue is considered harmless and orthogonal to this proposal.
