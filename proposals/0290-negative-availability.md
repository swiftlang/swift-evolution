...
Additionally, we were unable to locate concrete examples where this confusion could *actually* cause the feature to be misused. This is because if someone *does* misunderstand the branches, the project would simply fail to compile as there are symbols being used outside of an availability range. This would make it clear for the developer that their code is upside-down, and was the case even when making the statements as vague as possible in an attempt to introduce a bug on purpose.
