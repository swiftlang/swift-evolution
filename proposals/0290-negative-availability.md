...
Additionally, we were unable to locate concrete examples where this confusion could *actually* cause the feature to be misused. This is because even when we tried to craft a perfect example that would cause the statement to behave incorrectly in a specific platform, the typechecker was still preventing the project from compiling by diagnosing that there are symbols being used outside of an availability range, which would make it clear for the developer the branches were swapped.
