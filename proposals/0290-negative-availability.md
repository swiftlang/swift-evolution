...
Additionally, we were unable to locate concrete examples where this confusion could *actually* cause the feature to be misused. This is because even when we tried to craft a perfect example that would cause the statement to behave incorrectly in a specific platform, the typechecker was still preventing the project from compiling. In the event that someone *does* confuse the branches, the typechecker will still diagnose that the symbols are being used outside of an availability range, which should make it clear for the developer that they swapped the branches.
