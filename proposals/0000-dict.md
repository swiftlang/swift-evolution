...
- Suggested Improvements:
    ...
    - [`Dictionary.remove(at:)` returns next index](#5-dictionaryremoveat-returns-next-index)
    - [A `grouped(by:)` method for sequences](#6-a-groupedby-method-for-sequences)
    - [Relevant changes to `Set`](#7-apply-relevant-changes-to-set)
...
- An earlier version of this proposal declared the first initializer as failable, returning `nil` when a sequence with duplicate keys was passed. That initializer is really only appropriate when working with know unique keys, however, as there isn't a feasible recovery path when there can be duplicate keys. That leaves two possibilities:
...
If the `remove(at:)` method returns the next valid index, this kind of algorithm is possible. `remove(at:)` currently returns the key/value pair that was removed, so this would change the return type to a tuple of the removed key/value pair and the next valid index:
...
[sandbox]: http://swift.sandbox.bluemix.net/#/repl/58e2d14fbe294557dd143b76
