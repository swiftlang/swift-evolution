...
Adds a cheap, stable, process-unique identifier for the currently executing `Task`. `Task.currentID`, with instance accessors on `Task` and `UnsafeCurrentTask`, returns an opaque `Task.ID` at a cost comparable to a thread-local read — enabling always-on instrumentation like tracing and structured logging on the hot path.
...
```swift
// Constrained to Task<Never, Never> so callers can write `Task.currentID`
...
extension Task where Success == Never, Failure == Never {
  ...
  public static var currentID: Task.ID? { get }
}
```

A new opaque value type is introduced as the result type. It is declared at top level and surfaced through a typealias on `Task` so that `Task.ID` is one canonical type rather than a family parameterised over `Task`'s generic arguments:
...
The `Task.ID` / `task.id` spelling deliberately mirrors Swift's `Identifiable` protocol convention, so the names line up with what users already expect from identity-bearing types in the standard library.
...
`Task.ID` is an opaque value type wrapping a 64-bit unsigned integer produced by the runtime. The underlying integer value is accessible through `rawValue` for serialisation. No public initialiser is provided; the rationale for both choices is in *Alternatives considered*.
...
The struct is declared at top level rather than nested inside `Task`. Because `Task` is generic over `Success` and `Failure`, a directly-nested `Task.ID` would be `Task<Success, Failure>.ID` — a separate type per instantiation, which is the wrong model for a single canonical identifier type. The typealias gives the user-facing `Task.ID` spelling while keeping one canonical underlying type.

A consequence of this shape is that both `Task.ID` and the underlying `TaskID` resolve to the same type and are spellable at call sites. `Task.ID` is the canonical, documented spelling and should be preferred; `TaskID` exists as the unparameterised backing type because Swift's name resolution does not support nesting a non-generic type publicly inside a generic enclosing type.

The type serves two distinct usage shapes. **In-process consumers** — dictionaries keyed by task identity, sets of live tasks, typed API parameters carrying a task identifier across module boundaries — work with `Task.ID` directly and never reach for the underlying integer. **Wire-format and cross-language consumers** extract `rawValue` to put a stable 64-bit integer on the wire. The API is shaped to be ergonomic for both.

`rawValue` is the one supported escape hatch from the wrapper. Wire-format authors emit `rawValue` on the producer side and treat the value as an opaque integer on the consumer side; the consumer typically does not need to reconstruct a `Task.ID` instance to log it, key on it, or join it against other data. Serialised values are intended for exporting observations (logs, traces, joins against other data) — not for asserting task identity in another process, where the value carries no semantic guarantees beyond being a 64-bit integer. `MemoryLayout<Task.ID>.size` is a compile-time constant of `8`, fixed by `@frozen`.
...
The constraint mirrors how `Task.sleep` and similar static members are declared today: the static accessor does not depend on `Success` or `Failure`, so binding it to the canonical `Task<Never, Never>` namespace lets callers write `Task.currentID` without specifying generic arguments.
...
`Task.id` is always safe to call, including after the task has completed. The identifier is fixed at task creation and never changes thereafter; no synchronisation is required.

`UnsafeCurrentTask.id` carries the same safety contract as every other property on `UnsafeCurrentTask`: the unsafe task reference does not keep the task alive, and reading from a deallocated task is undefined behaviour. Because the property returns a trivial value type, callers can read the identifier inside the `withUnsafeCurrentTask` closure and continue to use the resulting value after the closure has exited.
...
- The steady-state cost of `Task.currentID` is in the single-digit nanosecond range on contemporary Apple Silicon, with comparable cost on other modern 64-bit hardware. The accessor should be no more expensive than a thread-local read.
...
- The synthesised `Equatable` and `Hashable` operations on `Task.ID` compile to the same instruction sequences as their `UInt64` equivalents for concrete-typed call sites.

The specific inlining strategy, ABI symbol shape, and back-deployment floor are matters for implementation review.

`Task.ID` is `@frozen` to pin its layout: this is part of the API contract, not an inlining hint. The `rawValue` accessor and the `MemoryLayout<Task.ID>.size == 8` guarantee both depend on it. The 64-bit width is similarly part of the contract via `rawValue`. Internal *interpretation* of those bits — for example, partitioning into a `(generation, slot)` pair, or mixing in a process-startup nonce — remains a free implementation choice as long as the semantic guarantees stated under [`Task.ID`](#taskid) (process-unique, never reused) continue to hold.
...
As noted under [`Task.ID`](#taskid), ordering is not part of the API contract and the type does not conform to `Comparable`. A conformance could be considered in a separate proposal if a use case emerges.
...
### Return a non-optional `Task.ID` with a reserved sentinel

`Task.currentID` is specified to return `Task.ID?`, where `nil` signals that no Swift `Task` is currently running on this thread. Because `UInt64` has no spare bits, on contemporary 64-bit Swift ABIs `Optional<Task.ID>` is 16 bytes and is returned in two registers; an unconditional `Task.ID` would be 8 bytes and returned in one. On the hot path this is a small but real difference.

An alternative is to reserve a value within `Task.ID` — most naturally zero, since the runtime never assigns it today — to mean "no current task", and to have the accessor return non-optional `Task.ID`. This trades a small amount of static type-system safety (callers must check `id != .none` rather than unwrap an `Optional`) for a measurably cheaper return convention and a slightly simpler call site for the common case where the caller does not care about the absent case (it just stores whatever it gets).

We chose `Task.ID?` for the proposal because it is the idiomatic Swift shape for "may not be present", and because the absent case is not entirely uncommon (synchronous callers from outside the concurrency runtime). The sentinel form remains available as a future addition: a separately-named non-optional accessor could be introduced without affecting `currentID`. This is one of the affordances a wrapper type enables that a bare `UInt64?` would not.
