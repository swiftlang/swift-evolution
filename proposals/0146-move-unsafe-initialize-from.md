...
If the collection's `count` ends up being lower than the actual number of elements returned later by its iterator, the calling collection may not allocate enough memory. Since `UnsafeMutableBufferPointer.initialize(from:)` does not receive a limiting count, this method would then scribble past the end of the buffer, resulting in undefined behaviour.
