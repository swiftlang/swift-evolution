...
* Proposal: [SE-0542](0542-package-manager-build-tool-plugin-usage-conditionals.md)
...
SwiftPM can evaluate the host and trait filters before it invokes the build tool plugin. A target-platform filter works differently. The build tool plugin and the Project Interchange Format (PIF) do not use the build request. Thus, the build tool plugin does not know the target platform. SwiftPM applies the target-platform filter to the commands that the build tool plugin returns and the outputs that those commands produce. The build system then uses the filter for each configured target.
...
3. **Filter commands and outputs.** SwiftPM applies the `platforms` filter to each `Command` from this plugin usage. Build commands become filtered `CustomTask` values in the PIF. Generated source and resource entries use the same filter. The build system selects them for each configured target.
...
SwiftPM applies the target-platform condition to all commands from the plugin usage. On an excluded platform, SwiftPM does not add the generated files to the target. The target must compile without these files. To use a different generator, add a separate plugin usage with its own condition:
